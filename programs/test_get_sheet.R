...
blockedMacAddressTable <- addressList %>% filter(ID=="excluded") %>% .$Item %>% read_sheet(sheet="blockedDevices")
dummy <- c("fortiGateUserInfo","sinetTable", "staticIpTable", "blockedMacAddressTable") %>% map( ~ TableWriteJson(.))
