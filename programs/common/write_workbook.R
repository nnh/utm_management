#' Utility Functions for Generating Excel Workbooks
#' This script provides functions to create and style Excel workbooks using the `openxlsx` package.
#' @file write_workbook.R
...
WriteDataToWorkbook <- function(wb, sheetname, table, currentRow) {
  ...
  for (i in seq_along(table)) {
    outputDf <- table[[i]]
    names(table)[[i]] %>% writeData(wb, sheet = sheetname, x = ., withFilter = FALSE, startRow = outputRow, sep = "\t")
    addStyle(wb, sheet = sheetname, kHeaderStyle, rows = outputRow, cols = 1)
    outputRow <- outputRow + 1
    outputDf %>% writeData(wb, sheet = sheetname, x = ., withFilter = FALSE, startRow = outputRow, sep = "\t")
...
CreateOutputWorkbook <- function(tables) {
  ...
  for (i in seq_along(tables)) {
    ...
    outputReportName %>% writeData(output_wb, sheet = i, x = ., withFilter = FALSE, startRow = outputRow, sep = "\t")
    addStyle(output_wb, sheet = i, kBodyStyle, rows = 1:110, cols = 1:8, gridExpand = TRUE)
    addStyle(output_wb, sheet = i, kTitleStyle, rows = outputRow, cols = 1)
    pageSetup(output_wb, sheet = i, orientation = "landscape", fitToWidth = TRUE, fitToHeight = FALSE)
  ...
  saveWorkbook(output_wb, str_c(output_path, "/", kOutputFilename, yyyymm, ".xlsx"), overwrite = TRUE)
}
# ------ main ------
