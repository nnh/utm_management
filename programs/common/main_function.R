...
GetIpAddressesAndDomains <- function(tables) {
  tables_vec <- tables %>%
    ...
    unlist()
  ipAddresses <- tables_vec %>%
    str_extract(kIpAddr) %>%
    na.omit() %>%
    unique() %>%
    tibble(ip = .)
  domains <- tables_vec %>%
    str_extract("[a-z0-9.-]+\\.[a-z]{2,}") %>%
    na.omit() %>%
    unique() %>%
    tibble(ip = .)
...
JoinReportAndUserInfoByTable <- function(tables, tableInfo) {
  ...
  for (i in seq_along(targetItemAndColumn)) {
    ...
    if (nrow(targetTable[[itemName]]) > 0) {
      if (tableName == kAdminAndSystemEvents) {
        targetTable[[itemName]][[key]] <- targetTable[[itemName]] %>%
          .[, columnName, drop = TRUE] %>%
          str_extract(KIpAddrPart)
      }
      if (tableName == kUserReport && !is.null(columnName)) {
        targetTable[[itemName]][[key]] <- targetTable[[itemName]] %>%
          .[, columnName, drop = TRUE] %>%
          str_extract(KIpAddrPart)
        usage_str <- targetTable[[itemName]] %>%
          .[, columnName, drop = TRUE] %>%
          str_extract(str_c(kUsageStr, "[0-9.]+ [A-Za-z]+")) %>%
          str_remove(kUsageStr)
        rank_str <- targetTable[[itemName]] %>%
          .[, columnName, drop = TRUE] %>%
          str_split_i(":", 1)
...
JoinUserInfo <- function(ipAddresses, deviceList) {
  ...
  userDomainInfo <- domainList$ip %>% map_df(~ {
    ...
    for (i in seq_len(nrow(deviceDomainList))) {
...
SetTableInfo <- function() {
  ...
  target[[2]]$targetItemAndColumn <- list(
    ...
    list(
      itemName = "Top 30 Destination by Bandwidth and Sessions", columnName = NULL,
      key = c("Hostname_or_IP_" = "ip")
    )
  )
...
GetWhoisInfo <- function(userInfo) {
  whois <- ext_path %>%
    file.path("whois.json") %>%
    fromJSON()
  sinet_ip <- addressList %>%
    filter(ID == "sinet_ip_list") %>%
    .$Item %>%
    trimws()
  sinet_item_name <- addressList %>%
    filter(ID == "sinet_item_name") %>%
    .$Item %>%
    trimws()
  sinet <- tibble(User = sinet_item_name, ip = sinet_ip)
  whois <- sinet %>% bind_rows(whois, .)
  hgc_ip <- addressList %>%
    filter(ID == "hgc_ip_list") %>%
    .$Item %>%
    trimws()
  hgc_item_name <- addressList %>%
    filter(ID == "hgc_item_name") %>%
    .$Item %>%
    trimws()
  hgc <- tibble(User = hgc_item_name, ip = hgc_ip)
  whois <- hgc %>% bind_rows(whois, .)
  nmc_ip <- addressList %>%
    filter(ID == "nmc_ip_list") %>%
    .$Item %>%
    trimws()
  ...
  targetIpList <- userInfo %>%
    filter(is.na(hostName) & str_detect(ip, kIpAddr)) %>%
    select("ip") %>%
    unlist()
  targetOct1 <- targetIpList %>%
    map_chr(~ str_split_i(., "\\.", 1)) %>%
    unique()
  ipAndUserList <- targetIpList %>%
    map(~ {
      input_ip <- .
      ...
      for (i in seq_len(nrow(targetWhois))) {
